                ...
                {#each (sidebarNavSource?.items || []).filter((sni) => sni.enabled) as sni, i}
                  {#if i < 5 || showAllSidebarItems}
                    {#if sidebarNavSource.mode !== "mini" || sni.id === "sn_home"}
                      ...
                      {:else if sni.id === "sn_vip" || sni.id === "sn_vip2" || sni.id === "sn_vip3"}
                        {@const viewId = sni.id.replace('sn_', '')}
                        <a
                          href="#"
                          class="nav-item {activeView === viewId ? 'active' : ''}"
                          on:click|preventDefault={() => setView(viewId)}
                          style="color: {activeView === viewId
                            ? 'var(--text-main)'
                            : 'var(--accent-gold)'};"
                          title={sni.label}
                        >
              ...
              {:else if bannersConfig.layout === "1_big_2_small"}
                ...
                <div class="banner-stack">
                  {#if small1Items.length > 0}
                    <div class="banner-slider-container banner-small" style="position: relative; overflow: hidden; border-radius: var(--card-radius); border: 1px solid rgba(255, 255, 255, 0.1); width: 100%; height: 100%;">
                      ...
                      {/each}
                      <div class="banner banner-small" style="opacity: 0; pointer-events: none;"></div>
